...
class Patient {
  ...
  int ageAt([DateTime? on]) {
    ...
    final hadBirthday = reference.month > birthDate.month ||
  ...
  factory Patient.fromJson(Map<String, dynamic> json) => Patient(
        ...
        nationalNumber:
            asStringOrNull(json['national_number'] ?? json['nationalNumber']),
        ...
        deceasedDate:
            asDateTimeOrNull(json['deceased_date'] ?? json['deceasedDate']),
  ...
  Map<String, dynamic> toFhir() => pruneNulls(<String, dynamic>{
        ...
        'telecom': <dynamic>[
          ...
          if (email != null)
            <String, dynamic>{'system': 'email', 'value': email},
        ],
        'gender': gender.name,
  ...
  factory Patient.fromFhir(Map<String, dynamic> resource) {
    ...
    String? findIdentifier(String typeCode) {
      for (final identifier in identifiers) {
        final codings =
            asMapList((identifier['type'] as Map?)?['coding']);
    ...
    String? findTelecom(String system) {
      for (final telecom in telecoms) {
        if (telecom['system'] == system) return asStringOrNull(telecom['value']);
      }
      return null;
    }

    final addresses = asMapList(resource['address']);
    final addr = addresses.isNotEmpty ? addresses.first : const <String, dynamic>{};

    final communications = asMapList(resource['communication']);
    String language = 'fr';
    if (communications.isNotEmpty) {
      final codings =
          asMapList((communications.first['language'] as Map?)?['coding']);
      if (codings.isNotEmpty) language = asString(codings.first['code'], fallback: 'fr');
    }
