...
class Encounter {
    ...
    bool clearLocation = false,
      ...
      Encounter(
        ...
        attendingPractitioner:
            attendingPractitioner ?? this.attendingPractitioner,
  ...
  factory Encounter.fromJson(Map<String, dynamic> json) => Encounter(
        ...
        dischargeDate:
            asDateTimeOrNull(json['discharge_date'] ?? json['dischargeDate']),
  ...
  factory Encounter.fromFhir(Map<String, dynamic> resource) {
    ...
    final period = (resource['period'] as Map?)?.cast<String, dynamic>() ??
        const <String, dynamic>{};
    final locations = asMapList(resource['location']);
    String? bedId;
    if (locations.isNotEmpty) {
      final reference =
          asString((locations.first['location'] as Map?)?['reference']);
      bedId = reference.startsWith('Location/') ? reference.substring(9) : null;
    }
    return Encounter(
      id: asString(resource['id']),
      patientId: subject.startsWith('Patient/') ? subject.substring(8) : subject,
      status: EncounterStatus.fromFhir(asStringOrNull(resource['status'])),
      encounterClass:
          EncounterClass.fromCode(asStringOrNull((resource['class'] as Map?)?['code'])),
